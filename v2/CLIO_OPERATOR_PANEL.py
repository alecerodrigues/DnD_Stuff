...
class FreqState:
    ...
    def __init__(self, board_size: int, frequency: int, turns: int, operator: int):
        ...
        self.color_order = []
        self.alt_point_set_one = []
        self.alt_point_set_two = []
    ...
    def gen_alt_tiles(self):
        random.seed(self.frequency * 2)
        i = 0
        k = 0
        while i < self.turns:
            x = random.randint(0, self.board_size - 1)
            ...
            if self.valid_tiles.__contains__(pt_set):
                ...
                self.alt_point_set_one.append(pt_set)
                i += 1
        ...
        while k < self.turns:
            ...
            if self.valid_tiles.__contains__(pt_set):
                ...
                self.alt_point_set_two.append(pt_set)
                k += 1
...
def control_PANEL():
    ...
    frequency_selection_PANEL(operation_value)
    pygame.quit()


def frequency_selection_PANEL(operator: int):
    BLACK = (0, 0, 0)
    WHITE = (255, 255, 255)
    pygame.init()
    clock = pygame.time.Clock()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if increase_freq.collidepoint(mouse_pos):
                    ...
                    # CHANGE INITS HERE
                    start_state = FreqState(7, int(frequency), 7, operator)
                    start_state.gen_valid_tiles()
                    start_state.gen_color_order()
                    start_state.gen_alt_tiles()
                    pygame.quit()
...
def memory_sequence_PANEL(frequency_state: FreqState):
    ...
    while menu:
        ...
        for event in pygame.event.get():
            ...
            possible_cores = []
            if frequency_state.operator == 1:  # RED
                RESET_COLOR = RED
                if frequency_state.color_order.__getitem__(current_turn - 1) == RED:
                    possible_cores = [frequency_state.valid_tiles.__getitem__(current_turn - 1),
                                      frequency_state.alt_point_set_one.__getitem__(current_turn - 1)]
                elif frequency_state.color_order.__getitem__(current_turn - 1) == BLUE:
                    possible_cores = [frequency_state.alt_point_set_two.__getitem__(current_turn - 1),
                                      frequency_state.alt_point_set_one.__getitem__(current_turn - 1)]

            elif frequency_state.operator == 2:  # BLUE
                RESET_COLOR = BLUE
                if frequency_state.color_order.__getitem__(current_turn - 1) == RED:
                    possible_cores = [frequency_state.alt_point_set_two.__getitem__(current_turn - 1),
                                      frequency_state.alt_point_set_one.__getitem__(current_turn - 1)]
        ...
        for y in range(frequency_state.board_size):
            for x in range(frequency_state.board_size):
                ...
                if correct_selected_memory_cores.__contains__((x + 1, y + 1)):
                    color = PURPLE
                if possible_cores.__contains__((x, y)):
                    color = GREEN
                if frequency_state.COMPLETE_FLAG:
                    RESET_COLOR = GREEN
